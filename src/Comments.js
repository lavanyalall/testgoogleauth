...
function Comments({ mainContract, userManagementContract, web3, ethAccount }) {
  ...
  const loadBlockchaindata = async () => {
    const encryptedPassword = await userManagementContract.methods
      .getUserPassword(ethAccount)
      .call();
    ...
    for (let i = 0; i < comments.length; i++) {
      const currSenderTrustScore = await mainContract.methods
        .getUserTrustScore(comments[i].from.ethAddress)
        .call();
      currTrustScores[comments[i].from.ethAddress] = currSenderTrustScore;
      const currReceiverTrustScore = await mainContract.methods
        .getUserTrustScore(comments[i].to.ethAddress)
        .call();
  ...
  return (
    <>
      {comments.length ? (
              ...
              <tbody>
                {comments.map((comment) => (
                  <tr key={comment.id}>
                    ...
                    <td>
                      {String(
                        Number(trustScores[comment.from.ethAddress]) / 100000
                      )}
                    </td>
                    <td>{String(comment.to.email)}</td>
                    <td>
                      {String(
                        Number(trustScores[comment.to.ethAddress]) / 100000
                      )}
                    </td>
        ...
        <h2 className="page-heading">No comments found</h2>
      )}
    </>
  );
}
